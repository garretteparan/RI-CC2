...
#include "ri_eomip_unr_energy_r.h"
...
void ri_eomip_unr_energy_r<TC,TI>::perform(array_view<TC> av_buff, multi_array<TC> ma_c, multi_array<TC> ma_oe,
                                  Mat<TC> &t1a, Mat<TC> &t1b, Row<TC> &r1a, Row<TC> &r1b, Row<TC> &res_a, Row<TC> &res_b,
                                  double &ene_exci, arma::vec Ea, arma::vec Eb, double c_os, double c_ss,
                                  rowvec &sigma_a, rowvec &sigma_b, unsigned algo) {
    // Check prereqs
    if(m_scf_type != scf_type::RSCF && m_scf_type != scf_type::USCF)
        throw std::runtime_error(" ri_eomip_unr_energy(): Unknown SCF type!");
    ...
    if(av_buff.size() < required_memory()) 
        throw std::runtime_error(" ri_eomip_unr_energy(): requires more memory!");
    ...
    if (algo == -1) {
        ri_eomip_unr_r<TC,TI>(m_reg).ccs_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                            BQov_a, BQvo_a, BQhp_a, BQoh_a, BQho_a, BQoo_a, 
                                                            BQov_b, BQvo_b, BQhp_b, BQoh_b, BQho_b, BQoo_b, 
                                                            f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, res_a, res_b, 
                                                            Eacta, Eactb, m_dev, m_b3, c_os, c_ss, sigma_a, sigma_b);
    } else if (algo == 0) {
        ri_eomip_unr_r<TC,TI>(m_reg).davidson_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                            BQov_a, BQvo_a, BQhp_a, BQoh_a, BQho_a, BQoo_a, 
                                                            BQov_b, BQvo_b, BQhp_b, BQoh_b, BQho_b, BQoo_b, 
                                                            f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, res_a, res_b, 
                                                            Eacta, Eactb, m_dev, m_b3, c_os, c_ss, sigma_a, sigma_b);
    } else if (algo == 1) {
        ri_eomip_unr_r<TC,TI>(m_reg).diis_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                            BQov_a, BQvo_a, BQhp_a, BQoh_a, BQho_a, BQoo_a, 
                                                            BQov_b, BQvo_b, BQhp_b, BQoh_b, BQho_b, BQoo_b, 
                                                            f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, res_a, res_b, 
                                                            Eacta, Eactb, m_dev, m_b3, c_os, c_ss, sigma_a, sigma_b);
...
template<typename TC, typename TI>
void ri_eomip_unr_energy_r<TC,TI>::compute_BQmn(arma::Mat<TC> &BQmn,
...
template<typename TC, typename TI>
size_t ri_eomip_unr_energy_r<TC,TI>::required_memory() {
    ...
    return 4*novx_a + 3*noox_a + 4*novx_b + 3*noox_b;
    
}
    
template<typename TC, typename TI>
size_t ri_eomip_unr_energy_r<TC,TI>::required_buffer_size() {
...
template<typename TC, typename TI>
size_t ri_eomip_unr_energy_r<TC,TI>::required_min_dynamic_memory() {
...
template class ri_eomip_unr_energy_r<double, double>;
// template class ri_eomip_unr_energy_r<std::complex<double> >;     // GPP: activate complex later
