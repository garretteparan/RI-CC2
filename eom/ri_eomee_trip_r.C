...
/// GPP: OPTIMIZE CODE
template<>
void ri_eomee_trip_r<double,double>::ccs_restricted_energy_triplets(
    ...
    Mat<double> &t1, Mat<double> &r1, Col<double> &e_orb, 
...
template<>
void ri_eomee_trip_r<double,double>::davidson_restricted_energy_triplets(
    double& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<double> &BQov_a, Mat<double> &BQvo_a, 
    Mat<double> &BQhp_a, Mat<double> &BQoh_a, 
    ...
    Col<double> &e_orb, array_view<double> av_pqinvhalf,
    const libqints::dev_omp &m_dev,
    const libqints::basis_2e3c_shellpair_cgto<double> &m_b3,
    double c_os, double c_ss, Mat<double> &sigma) {
    ...
    
    {
        ...
        #pragma omp parallel
        {
            ...
            for(size_t ij = 0; ij < npairs; ij++) {
                ...
                arma::Mat<double> W3 = Bbp_i.st() * Bhp_j; // r2:   a/ibj
                arma::Mat<double> W4 = Bbp_j.st() * Bhp_i; // r2:   b/jai
                ...
                if(i == j) {
                    ...
                    for(size_t b = 0; b < n_vir; b++) {
                        ...
                        for(size_t a = 0; a < n_vir; a++) {
                            ...
                            // r2ab = (w1b[a] + w3b[a] + w1b[a] + w3b[a] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a]) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);
                            // r2ab = (c_os*(w1b[a] + w3b[a]) + c_ss*(w1b[a] + w3b[a] - w1[a*n_vir+b] - w3[a*n_vir+b]) - c_ss*(w2b[a] - w4b[a])) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);

                            r2ab = (c_os*(w1b[a] + w3b[a]) + c_ss*(0.5*(w1b[a] + w3b[a]) - w1[a*n_vir+b] - w3[a*n_vir+b]) + c_ss*(0.5*(w1b[a] + w3b[a]) - w2b[a] - w4b[a])) 
                                                / (dijb - e_orb[n_occ+a] + exci);
                            ...
                            for(size_t Q = 0; Q < n_aux; Q++) {
                                ...
                                Y_bar_local[(a*n_occ*n_aux+i*n_aux+Q)] += r2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                            }

                            // sigma_I(a,i) += (r2ab * Fov_hat(j,b)) - (t2ba * Fov_bar(j,b));
                            sigma_I_local(a,i) += (r2ab * Fov_hat(j,b)) - c_ss*(t2ba * Fov_bar(j,b));
                    ...
                    for(size_t b = 0; b < n_vir; b++) {
                        ...
                        for(size_t a = 0; a < n_vir; a++) {
                            ...
                            // r2ab = (w1b[a] + w3b[a] + w1b[a] + w3b[a] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a]) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);
                            // r2ba = (w2[a*n_vir+b] + w4[a*n_vir+b] + w2[a*n_vir+b] + w4[a*n_vir+b] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a]) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);

                            // r2ab = (c_os*(w1b[a] + w3b[a]) + c_ss*(w1b[a] + w3b[a] - w1[a*n_vir+b] - w3[a*n_vir+b]) - c_ss*(w2b[a] - w4b[a])) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);
                            // r2ba = (c_os*(w2[a*n_vir+b] + w4[a*n_vir+b]) + c_ss*(w2[a*n_vir+b] + w4[a*n_vir+b] - w2b[a] - w4b[a]) - c_ss*(w1[a*n_vir+b] - w3[a*n_vir+b])) 
                            //                     / (dijb - e_orb[n_occ+a] + exci);


                            r2ab = (c_os*(w1b[a] + w3b[a]) + c_ss*(0.5*(w1b[a] + w3b[a]) - w1[a*n_vir+b] - w3[a*n_vir+b]) + c_ss*(0.5*(w1b[a] + w3b[a]) - w2b[a] - w4b[a])) 
                                                / (dijb - e_orb[n_occ+a] + exci);
                            r2ba = (c_os*(w2[a*n_vir+b] + w4[a*n_vir+b]) + c_ss*(0.5*(w2[a*n_vir+b] + w4[a*n_vir+b]) - w2b[a] - w4b[a]) + c_ss*(0.5*(w2[a*n_vir+b] + w4[a*n_vir+b]) - w1[a*n_vir+b] - w3[a*n_vir+b])) 
                                                / (dijb - e_orb[n_occ+a] + exci);
                            ...
                            for(size_t Q = 0; Q < n_aux; Q++) {
                                ...
                                // Y_bar[(a*n_occ*n_aux+i*n_aux+Q)] += r2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Y_bar[(b*n_occ*n_aux+j*n_aux+Q)] += r2ba * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)];
                                // Yia_local[(a*n_occ*n_aux+i*n_aux+Q)] += (2.0*t2ab-t2ba) * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Yia_local[(b*n_occ*n_aux+j*n_aux+Q)] += (2.0*t2ab-t2ba) * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)];
                                // Yai_local[(i*n_vir*n_aux+a*n_aux+Q)] += (2.0*t2ab-t2ba) * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Yai_local[(j*n_vir*n_aux+b*n_aux+Q)] += (2.0*t2ab-t2ba) * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)];
                                Yia_local[(a*n_occ*n_aux+i*n_aux+Q)] += c_os * (t2ab) * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)]
                                ...
                                Yai_local[(j*n_vir*n_aux+b*n_aux+Q)] += c_os * (t2ab) * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)]
                                                                        + c_ss * (t2ab-t2ba) * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)];
                                Y_bar_local[(a*n_occ*n_aux+i*n_aux+Q)] += r2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                Y_bar_local[(b*n_occ*n_aux+j*n_aux+Q)] += r2ba * BQov_a[(i*n_vir*n_aux+a*n_aux+Q)];
                            }
                            
                            // sigma_I(a,i) += (r2ab * Fov_hat(j,b)) - (t2ba * Fov_bar(j,b));
                            // sigma_I(b,j) += (r2ba * Fov_hat(i,a)) - (t2ba * Fov_bar(i,a));
                            sigma_I_local(a,i) += (r2ab * Fov_hat(j,b)) - c_ss*(t2ba * Fov_bar(j,b));
                            sigma_I_local(b,j) += (r2ba * Fov_hat(i,a)) - c_ss*(t2ba * Fov_bar(i,a));
                        }
                    }
                }
            }
...
template<>
void ri_eomee_trip_r<double,double>::diis_restricted_energy_triplets(
    double& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<double> &BQov_a, Mat<double> &BQvo_a, 
    Mat<double> &BQhp_a, Mat<double> &BQoh_a, 
                    ...
                    for(size_t b = 0; b < n_vir; b++) {
                        ...
                        for(size_t a = 0; a < n_vir; a++) {
                            ...
                            for(size_t Q = 0; Q < n_aux; Q++) {
                                ...
                                // Yai[(i*n_vir*n_aux+a*n_aux+Q)] += t2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Y_bar[(a*n_occ*n_aux+i*n_aux+Q)] += r2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Yia_local[(a*n_occ*n_aux+i*n_aux+Q)] += t2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                // Yai_local[(i*n_vir*n_aux+a*n_aux+Q)] += t2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                Yia_local[(a*n_occ*n_aux+i*n_aux+Q)] += c_os * t2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                Yai_local[(i*n_vir*n_aux+a*n_aux+Q)] += c_os * t2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                                Y_bar_local[(a*n_occ*n_aux+i*n_aux+Q)] += r2ab * BQov_a[(j*n_vir*n_aux+b*n_aux+Q)];
                            }
...
template<>
void ri_eomee_trip_r<double,double>::ccs_restricted_energy_triplets_digestor(
    double& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<double> &BQov_a, Mat<double> &BQvo_a, 
    Mat<double> &BQhp_a, Mat<double> &BQoh_a, 
...
template<>
void ri_eomee_trip_r<double,double>::davidson_restricted_energy_triplets_digestor(
    double& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<double> &BQov_a, Mat<double> &BQvo_a, 
    Mat<double> &BQhp_a, Mat<double> &BQoh_a, 
                ...
                if(i == j) {
                    ...
                    for(size_t b = 0; b < n_vir; b++) {
                        ...
                        for(size_t a = 0; a < n_vir; a++) {
                            ...
                            // sigma_I(a,i) += (r2ab * Fov_hat(j,b)) - (t2ba * Fov_bar(j,b));
                            sigma_I_local(a,i) += (r2ab * Fov_hat(j,b)) - (t2ba * Fov_bar(j,b));
                    ...
                    for(size_t b = 0; b < n_vir; b++) {
                        ...
                        for(size_t a = 0; a < n_vir; a++) {
                            ...
                            // sigma_I(b,j) += (r2ba * Fov_hat(i,a)) - (t2ba * Fov_bar(i,a));
                            sigma_I_local(a,i) += (r2ab * Fov_hat(j,b)) - (t2ba * Fov_bar(j,b));
                            sigma_I_local(b,j) += (r2ba * Fov_hat(i,a)) - (t2ba * Fov_bar(i,a));
                        }
                    }
                }
            }
...
template<>
void ri_eomee_trip_r<double,double>::diis_restricted_energy_triplets_digestor(
    double& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<double> &BQov_a, Mat<double> &BQvo_a, 
    Mat<double> &BQhp_a, Mat<double> &BQoh_a, 
...
void ri_eomee_r<TC, TI>::restricted_energy_triplets(
    complex<double>& exci, const size_t& n_occ, const size_t& n_vir,
    const size_t& n_aux, const size_t& n_orb,
    Mat<TC> &BQov_a, Mat<TC> &BQvo_a, 
    Mat<TC> &BQhp_a, Mat<TC> &BQoh_a, Mat<TC> &BQho_a, 
    Mat<TC> &BQoo_a, Mat<TC> &BQob_a, Mat<TC> &BQpv_a, 
    Mat<TC> &BQpo_a, Mat<TC> &BQhb_a, Mat<TC> &BQbp_a, 
    Mat<TC> &V_Pab,  Mat<TC> &Lam_hA, Mat<TC> &Lam_pA,
    ...
    
    {
        ...
        complex<double> excit= (0.0, 0.0);
        
        // intermediates
        Mat<TC> sigma_0 (n_vir, n_occ, fill::zeros);
        Mat<TC> sigma_JG (n_vir, n_occ, fill::zeros);
        Mat<TC> sigma_H (n_vir, n_occ, fill::zeros);
        Mat<TC> sigma_I (n_vir, n_occ, fill::zeros);
        Mat<TC> E_vv (n_vir, n_vir, fill::zeros);
        Mat<TC> E_oo (n_occ, n_occ, fill::zeros);
        Mat<TC> Yai (n_aux, n_vir*n_occ, fill::zeros);
        Mat<TC> Yia (n_aux, n_vir*n_occ, fill::zeros);
        Mat<TC> Y_bar (n_aux, n_vir*n_occ, fill::zeros);
        Mat<TC> gamma_P (n_aux, n_orb*n_occ, fill::zeros);
        Mat<TC> JG (n_orb, n_occ, fill::zeros);
        Mat<TC> sigma (n_vir, n_occ, fill::zeros);
        Mat<TC> update (n_vir, n_occ, fill::zeros);
        
        /// step 3: form iQ, iQ_bar, F_ia, F_ab, F_ij
        Col<TC> iQ (n_aux, fill::zeros);
        Col<TC> iQ_bar (n_aux, fill::zeros);
        iQ += BQov_a * t1;
        iQ_bar += BQov_a * r1;

        // Fov_hat
        Mat<TC> F1 = 2.0 * iQ.st() * BQov_a;
        Mat<TC> F11(F1.memptr(), n_vir, n_occ, false, true);
        Mat<TC> Fov_hat1 = F11.st();
        Mat<TC> BQvo(BQvo_a.memptr(), n_aux*n_occ, n_vir, false, true);
        Mat<TC> BQoo(BQoo_a.memptr(), n_aux*n_occ, n_occ, false, true);
        Mat<TC> Fov_hat2 = BQoo.st() * BQvo;
        Mat<TC> Fov_hat = Fov_hat1 - Fov_hat2;

        // Fov_bar
        Mat<TC> F2 = 2.0 * iQ_bar.st() * BQov_a;
        Mat<TC> F22(F2.memptr(), n_vir, n_occ, false, true);
        Mat<TC> Fov_bar1 = F22.st();
        Mat<TC> BQob(BQob_a.memptr(), n_aux*n_occ, n_occ, false, true);
        Mat<TC> Fov_bar2 = BQob.st() * BQvo;
        Mat<TC> Fov_bar = - Fov_bar2; //triplet

        // Fvv_hat
        Mat<TC> F3 = 2.0 * iQ.st() * BQpv_a;
        Mat<TC> F33(F3.memptr(), n_vir, n_vir, false, true);
        Mat<TC> Fvv_hat1 = F33.st();
        Mat<TC> BQpo(BQpo_a.memptr(), n_aux*n_occ, n_vir, false, true);
        Mat<TC> Fvv_hat2 = BQpo.st() * BQvo;
        Mat<TC> Fvv_hat = f_vv + Fvv_hat1 - Fvv_hat2;

        // Foo_hat
        Mat<TC> F4 = 2.0 * iQ.st() * BQoh_a;
        Mat<TC> F44(F4.memptr(), n_occ, n_occ, false, true);
        Mat<TC> Foo_hat1 = F44.st();
        Mat<TC> BQho(BQho_a.memptr(), n_aux*n_occ, n_occ, false, true);
        Mat<TC> Foo_hat2 = BQoo.st() * BQho;
        Mat<TC> Foo_hat = f_oo + Foo_hat1 - Foo_hat2;
        ...
        for(size_t ij = 0; ij < npairs; ij++) {
            ...
            // for t2: 
            Mat<TC> Bhp_i(BQhp_a.colptr(i*n_vir), n_aux, n_vir, false, true);
            Mat<TC> Bhp_j(BQhp_a.colptr(j*n_vir), n_aux, n_vir, false, true);

            // for r2: 
            Mat<TC> Bhb_i(BQhb_a.colptr(i*n_vir), n_aux, n_vir, false, true);
            Mat<TC> Bhb_j(BQhb_a.colptr(j*n_vir), n_aux, n_vir, false, true);
            Mat<TC> Bbp_i(BQbp_a.colptr(i*n_vir), n_aux, n_vir, false, true);
            Mat<TC> Bbp_j(BQbp_a.colptr(j*n_vir), n_aux, n_vir, false, true);

            // integrals
            Mat<TC> W0 = Bhp_i.st() * Bhp_j; // t2:   aibj
            Mat<TC> W1 = Bhb_i.st() * Bhp_j; // r2:   /aibj
            Mat<TC> W2 = Bhb_j.st() * Bhp_i; // r2:   /bjai
            Mat<TC> W3 = Bbp_i.st() * Bhp_j; // r2:   a/ibj
            Mat<TC> W4 = Bbp_j.st() * Bhp_i; // r2:   b/jai
            
            TC delta_ij = e_orb(i) + e_orb(j);
            TC t2ab = (0.0, 0.0), t2ba = (0.0, 0.0),
               r2ab = (0.0, 0.0), r2ba = (0.0, 0.0);
            
            if(i == j) {
                const TC *w0 = W0.memptr();
                const TC *w1 = W1.memptr();
                const TC *w2 = W2.memptr();
                const TC *w3 = W3.memptr();
                const TC *w4 = W4.memptr();

                for(size_t b = 0; b < n_vir; b++) {
                    
                    const TC *w0b = w0 + b * n_vir;
                    const TC *w1b = w1 + b * n_vir;
                    const TC *w2b = w2 + b * n_vir;
                    const TC *w3b = w3 + b * n_vir;
                    const TC *w4b = w4 + b * n_vir;

                    TC dijb = delta_ij - e_orb[n_occ+b];

                    for(size_t a = 0; a < n_vir; a++) {
			            TC denom_t = dijb - e_orb[n_occ+a];   
		                TC denom_r = denom_t + exci; 	
                        t2ab = (conj(denom_t) * w0b[a]) / (conj(denom_t) * denom_t);
                        t2ba = (conj(denom_t) * w0[a*n_vir+b]) / (conj(denom_t) * denom_t);
                        
                        r2ab = (conj(denom_r) * (w1b[a] + w3b[a] + w1b[a] + w3b[a] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a])) 
                ...
                for(size_t b = 0; b < n_vir; b++) {
                    ...
                    for(size_t a = 0; a < n_vir; a++) {
                        ...
                        r2ab = (conj(denom_r) * (w1b[a] + w3b[a] + w1b[a] + w3b[a] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a])) 
                                            / (conj(denom_r) * denom_r);
                        r2ba = (conj(denom_r) * (w2[a*n_vir+b] + w4[a*n_vir+b] + w2[a*n_vir+b] + w4[a*n_vir+b] - w1[a*n_vir+b] - w2b[a] - w3[a*n_vir+b] - w4b[a])) 
                                            / (conj(denom_r) * denom_r);
        ...
        Mat<TC> YQia(Yia.memptr(), n_aux*n_occ, n_vir, false, true);
        Mat<TC> YQai(Yai.memptr(), n_aux*n_vir, n_occ, false, true);
        Mat<TC> BQov(BQov_a.memptr(), n_aux*n_vir, n_occ, false, true);
        E_vv = Fvv_hat - YQia.st() * BQvo; // E_ab
        E_oo = Foo_hat + (YQai.st() * BQov).st(); // E_ji
        ...
        // omega_G1: first term of Γ(P,iβ)
        Mat<TC> YQia_bar(Y_bar.memptr(), n_aux*n_occ, n_vir, false, true);
        Mat<TC> gamma_G1 = YQia_bar * CvirtA.st(); // (n_aux*n_occ,n_orb)
        Mat<TC> gamma_G = gamma_G1.submat( 0, 0, n_aux-1, n_orb-1 );
        for(size_t i = 1; i < n_occ; i++) {
            gamma_G.insert_cols(i*n_orb, gamma_G1.submat( i*n_aux, 0, (i+1)*n_aux-1, n_orb-1 ));
        }

        // omega_J1: second term of Γ(P,iβ)
        Mat<TC> gamma_J11 = 2.0 * iQ_bar * vectorise(Lam_hA).st();
        Mat<TC> gamma_J1(gamma_J11.memptr(), n_aux*n_occ, n_orb, false, true);

        // / omega_J2: third term of Γ(P,iβ)
        Mat<TC> BQoh(BQoh_a.memptr(), n_aux*n_occ, n_occ, false, true);
        Mat<TC> gamma_J22 = BQoh * (Lam_hA_bar).st(); // (n_aux*n_occ, n_orb)
        Mat<TC> gamma_J2 = gamma_J22.submat( 0, 0, n_aux-1, n_orb-1 );
        for(size_t i = 1; i < n_occ; i++) {
            gamma_J2.insert_cols(i*n_orb, gamma_J22.submat( i*n_aux, 0, (i+1)*n_aux-1, n_orb-1 ));
        }

        // combine omega_G and omega_J: full terms of Γ(P,iβ)
        Mat<TC> gamma_Q = gamma_G - gamma_J2; // triplet
        ...
        // c-product
        Mat<double> r1_real = real(r1);
        Mat<double> r1_imag = imag(r1);
        vec vecre = vectorise(r1_real);
        vec vecim = vectorise(r1_imag);
        double cnorm_re = dot(vecre, vecre) - dot(vecim, vecim);
        double cnorm_im = 2.0 * dot(vecre, vecim);
        TC cnorm(cnorm_re, cnorm_im);
        cnorm = sqrt(cnorm);
...
template class ri_eomee_trip_r<complex<double>, double>;
template class ri_eomee_trip_r<complex<double>, complex<double>>;
