...
#include "ri_eomea_unr_energy_r.h"
...
void ri_eomea_unr_energy_r<TC,TI>::perform(array_view<TC> av_buff, multi_array<TC> ma_c, multi_array<TC> ma_oe,
                                        Mat<TC> &t1a, Mat<TC> &t1b, Col<TC> &r1a, Col<TC> &r1b, double &ene_exci, 
                                        arma::vec Ea, arma::vec Eb, double c_os, double c_ss, Col<TC> &sigma_a, Col<TC> &sigma_b, 
                                        unsigned algo)
{
    // Check prereqs
    if(m_scf_type != scf_type::RSCF && m_scf_type != scf_type::USCF)
        throw std::runtime_error(" ri_eomea_unr_energy(): Unknown SCF type!");
    ...
    if(av_buff.size() < required_memory()) 
        throw std::runtime_error(" ri_eomea_unr_energy(): requires more memory!");
    ...
    arma::Mat<TC> CvirtB = Cb.cols(m_ns.occb, m_ns.occb + m_ns.vacb - 1); //(orb,vir)
    arma::Mat<TC> CoccB = Cb.cols(m_ns.ofzb, m_ns.occb - 1); //(orb,occ)
    ...
    arma::Mat<TC> f_ov_b (m_ns.oacb, m_ns.vacb, fill::zeros);
    arma::Mat<TC> f_vo_b (m_ns.vacb, m_ns.oacb, fill::zeros);
    ...
    f_ov_b = CoccB.st() * Fb * CvirtB;
    f_vo_b = Lam_pB.st() * Fb * Lam_hB;
    
    
    /// step 2: form the different B matrices
    // Initialize memory buffer for BQai
    multi_array<TC> ma_BQmn(13);
    // RSCF
    ...
    ma_BQmn.set(6, mem.alloc(novx_b)); // B_ov
    ma_BQmn.set(7, mem.alloc(novx_b)); // B_vo
    ma_BQmn.set(8, mem.alloc(novx_b)); // B_hp
    ma_BQmn.set(9, mem.alloc(novx_b)); // B_po
    ma_BQmn.set(10, mem.alloc(noox_b)); // B_oo
    ma_BQmn.set(11, mem.alloc(nvvx_b)); // B_vp
    ...
    arma::Mat<TC> BQov_b(arrays<TC>::ptr(ma_BQmn[6]), naux, m_ns.oacb * m_ns.vacb, false, true);
    arma::Mat<TC> BQvo_b(arrays<TC>::ptr(ma_BQmn[7]), naux, m_ns.oacb * m_ns.vacb, false, true);
    arma::Mat<TC> BQhp_b(arrays<TC>::ptr(ma_BQmn[8]), naux, m_ns.oacb * m_ns.vacb, false, true);
    arma::Mat<TC> BQpo_b(arrays<TC>::ptr(ma_BQmn[9]), naux, m_ns.oacb * m_ns.vacb, false, true);
    arma::Mat<TC> BQoo_b(arrays<TC>::ptr(ma_BQmn[10]), naux, m_ns.oacb * m_ns.oacb, false, true);
    arma::Mat<TC> BQvp_b(arrays<TC>::ptr(ma_BQmn[11]), naux, m_ns.vacb * m_ns.vacb, false, true);
    ...
    //BQvp
    compute_BQmn(BQvp_b, mem.alloc(nvvx_b), CvirtB, Lam_pB);
    mem.load_state(chkpt);
    ...
    if (algo == -1) {
        ri_eomea_unr_r<TC,TI>(m_reg).css_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                        BQov_a, BQvo_a, BQhp_a, BQoo_a, BQpo_a, BQvp_a, 
                                                        BQov_b, BQvo_b, BQhp_b, BQoo_b, BQpo_b, BQvp_b, 
                                                        Lam_hA, Lam_pA, Lam_hB, Lam_pB, CoccA, CvirtA, CoccB, CvirtB, 
                                                        f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, Eacta, Eactb,
                                                        av_buff_ao, m_av_pqinvhalf, m_dev, m_b3, c_os, c_ss, sigma_a, sigma_b);
    } else if (algo == 0) {
        ri_eomea_unr_r<TC,TI>(m_reg).davidson_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                        BQov_a, BQvo_a, BQhp_a, BQoo_a, BQpo_a, BQvp_a, 
                                                        BQov_b, BQvo_b, BQhp_b, BQoo_b, BQpo_b, BQvp_b, 
                                                        Lam_hA, Lam_pA, Lam_hB, Lam_pB, CoccA, CvirtA, CoccB, CvirtB, 
                                                        f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, Eacta, Eactb,
        ...
        ri_eomea_unr_r<TC,TI>(m_reg).diis_unrestricted_energy(ene_exci, m_ns.oaca, m_ns.vaca, m_ns.oacb, m_ns.vacb, naux, nbsf,
                                                        BQov_a, BQvo_a, BQhp_a, BQoo_a, BQpo_a, BQvp_a, 
                                                        BQov_b, BQvo_b, BQhp_b, BQoo_b, BQpo_b, BQvp_b, 
                                                        Lam_hA, Lam_pA, Lam_hB, Lam_pB, CoccA, CvirtA, CoccB, CvirtB, 
                                                        f_vv_a, f_oo_a, f_vv_b, f_oo_b, t1a, t1b, r1a, r1b, Eacta, Eactb,
...
template<typename TC, typename TI>
void ri_eomea_unr_energy_r<TC,TI>::compute_BQmn(arma::Mat<TC> &BQmn,
...
template<typename TC, typename TI>
size_t ri_eomea_unr_energy_r<TC,TI>::required_memory() {
    ...
    // if(m_scf_type == scf_type::USCF)
        return 4*novx_a + 4*novx_b + noox_a + noox_b + nvvx_a + nvvx_b + nrrx;
...
template<typename TC, typename TI>
size_t ri_eomea_unr_energy_r<TC,TI>::required_buffer_size() {
    ...
    // if(m_scf_type == scf_type::USCF)
        return 4*novx_a + 4*novx_b + noox_a + noox_b + nvvx_a + nvvx_b + nrrx;nrrx;
    // return 4*novx_a + noox_a + nvvx_a + nrrx;
    // return 4*novx_a + noox_a + 2*nvvx_a;
...
template<typename TC, typename TI>
size_t ri_eomea_unr_energy_r<TC,TI>::required_min_dynamic_memory() {
...
template class ri_eomea_unr_energy_r<double, double>;
// template class ri_eomea_unr_energy_r<std::complex<double> >;     // GPP: activate complex later
